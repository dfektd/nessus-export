...
[string]$response = Invoke-WebRequest -Uri $Url -Method Post -Headers $header -Body $body -SkipCertificateCheck
...
[string]$status = Invoke-WebRequest -Uri $statusURL -Method Get -Headers $header -SkipCertificateCheck
...
Invoke-WebRequest -Uri $exportURL -Method GET -Headers $header -Outfile .\${filename}_${file_id}.$format -SkipCertificateCheck
